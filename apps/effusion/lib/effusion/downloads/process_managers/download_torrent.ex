defmodule Effusion.Downloads.ProcessManagers.DownloadTorrent do
  ...
  def handle(
    ...
    Logger.debug(
      "***** Got block #{inspect({index, offset})}, storing it, BTW requests is #{
        inspect(requests)
      }"
    )
  ...
  def apply(
    ...
    %__MODULE__{download | connecting_to_peers: MapSet.put(connecting_to_peers, peer_uuid)}
  end
    ...
    %__MODULE__{
      ...
      | connecting_to_peers: MapSet.delete(connecting_to_peers, peer_uuid),
        failcounts:
          if(reason == "normal",
            do: failcounts,
            else: Map.update(failcounts, peer_uuid, 1, &(&1 + 1))
          )
    }
  end

  def apply(
        %__MODULE__{connecting_to_peers: connecting_to_peers, connected_peers: connected_peers} =
          download,
        %PeerConnected{peer_uuid: peer_uuid}
      ) do
  ...
  def apply(
    ...
    %__MODULE__{
      download
      | peer_bitfields:
          Map.put(peer_bitfields, peer_uuid, Base.decode64!(bitfield) |> IntSet.new())
    }
  end
  ...
  def apply(
    ...
    %__MODULE__{
      download
      | peer_bitfields:
          Map.update(peer_bitfields, peer_uuid, IntSet.new([index]), &IntSet.put(&1, index))
    }
  end

  def apply(%__MODULE__{pieces: pieces, blocks: blocks} = download, %PieceHashSucceeded{
        index: index
      }) do
    %__MODULE__{
      download
  ...
  def apply(download, %AllPiecesVerified{}) do
    %__MODULE__{download | status: "shutting down"}
  end
  ...
  def apply(
    ...
    %__MODULE__{
      ...
      | blocks: Map.update(blocks, index, IntSet.new(offset), &IntSet.put(&1, offset)),
        requests:
          Map.update(requests, {index, offset, size}, MapSet.new(), &MapSet.delete(&1, peer_uuid))
    }
  end
  ...
  defp at_connection_limit?(%__MODULE__{
         connected_peers: connected_peers,
         connecting_to_peers: connecting_to_peers,
         max_connections: max_connections,
         max_half_open_connections: max_half_open_connections
    ...
    conn_count + half_open_count >= max_connections or
      half_open_count >= max_half_open_connections
  end

  defp next_peer_connection(%__MODULE__{
         connected_peers: connected_peers,
         connecting_to_peers: connecting_to_peers,
  ...
  defp get_more_requests_for_peer(
    ...
    request_count_we_can_add = max_requests_per_peer - Enum.count(existing_requests_to_this_peer)

    # all pieces we have
    blocks_to_request =
      pieces
      # all pieces we need
      |> IntSet.inverse(target_piece_count)
      |> Stream.flat_map(fn required_piece_index ->
        blocks
        # all blocks we have in this piece index
        |> Map.get(required_piece_index, IntSet.new())
        # all blocks we need in this piece index
        |> IntSet.inverse(blocks_per_piece)
        |> Stream.map(fn required_block_index ->
          # the block we need to request
          {required_piece_index, required_block_index * block_size, block_size}
        end)
      end)
      # All requests for block that the peer has
      |> Stream.filter(fn {i, _o, _s} -> Enum.member?(pieces_peer_has, i) end)
      # All requests for blocks that the peer has that we haven't already requested
      |> Stream.reject(&Enum.member?(existing_requests_to_this_peer, &1))
    ...
    def decode(
      ...
      %Effusion.Downloads.ProcessManagers.DownloadTorrent{
        ...
        | pieces: IntSet.new(Base.decode64!(pieces)),
          ...
          peer_bitfields:
            Enum.map(peer_bitfields, fn {uuid, bitfield} ->
              {uuid, Base.decode64!(bitfield) |> IntSet.new()}
            end)
      }
    end
  end
